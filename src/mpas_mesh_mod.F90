...
contains
  ...
  subroutine mpas_mesh_output(DVT_array, DT_list, DE_array, VVT_array, VC_array, VE_array, tag)
    ...
    call io_add_att('mpas_mesh', 'mesh_id',       'demo')
    call io_add_att('mpas_mesh', 'mesh',          '1.0')
    ...
    call io_add_dim('mpas_mesh', 'vertexDegree', size=vertexDegree)
    call io_add_dim('mpas_mesh', 'maxEdges',     size=maxEdges)
    call io_add_dim('mpas_mesh', 'maxEdges2',    size=maxEdges2)
    call io_add_dim('mpas_mesh', 'TWO',          size=2)
    call io_add_dim('mpas_mesh', 'Time')
    ...
    call io_add_var('mpas_mesh', 'indexToCellID',     long_name='Global cell ID for all cell centers',                             units='',       dim_names=['nCells      '],                 data_type='integer')
    call io_add_var('mpas_mesh', 'latEdge',           long_name='Latitude of all edge locations',                                  units='radian', dim_names=['nEdges      '],                 data_type='real(8)')
    call io_add_var('mpas_mesh', 'lonEdge',           long_name='Longitude of all edge locations',                                 units='radian', dim_names=['nEdges      '],                 data_type='real(8)')
    call io_add_var('mpas_mesh', 'xEdge',             long_name='x axis position of all edge locations',                           units='m',      dim_names=['nEdges      '],                 data_type='real(8)')
    call io_add_var('mpas_mesh', 'yEdge',             long_name='y axis position of all edge locations',                           units='m',      dim_names=['nEdges      '],                 data_type='real(8)')
    call io_add_var('mpas_mesh', 'zEdge',             long_name='z axis position of all edge locations',                           units='m',      dim_names=['nEdges      '],                 data_type='real(8)')
    call io_add_var('mpas_mesh', 'indexToEdgeID',     long_name='Global edge ID for all edge locations',                           units='',       dim_names=['nEdges      '],                 data_type='integer')
    call io_add_var('mpas_mesh', 'latVertex',         long_name='Latitude of all cell vertices',                                   units='radian', dim_names=['nVertices   '],                 data_type='real(8)')
    call io_add_var('mpas_mesh', 'lonVertex',         long_name='Longitude of all cell vertices',                                  units='radian', dim_names=['nVertices   '],                 data_type='real(8)')
    call io_add_var('mpas_mesh', 'xVertex',           long_name='x axis position of all cell vertices',                            units='m',      dim_names=['nVertices   '],                 data_type='real(8)')
    call io_add_var('mpas_mesh', 'yVertex',           long_name='y axis position of all cell vertices',                            units='m',      dim_names=['nVertices   '],                 data_type='real(8)')
    call io_add_var('mpas_mesh', 'zVertex',           long_name='z axis position of all cell vertices',                            units='m',      dim_names=['nVertices   '],                 data_type='real(8)')
    call io_add_var('mpas_mesh', 'indexToVertexID',   long_name='Global vertex ID for all cell vertices',                          units='',       dim_names=['nVertices   '],                 data_type='integer')
    call io_add_var('mpas_mesh', 'nEdgesOnCell',      long_name='Number of edges on a given cell',                                 units='',       dim_names=['nCells      '],                 data_type='integer')
    call io_add_var('mpas_mesh', 'nEdgesOnEdge',      long_name='Number of edges on a given edge',                                 units='',       dim_names=['nEdges      '],                 data_type='integer')
    call io_add_var('mpas_mesh', 'cellsOnCell',       long_name='Cell indices that surround a given cell',                         units='',       dim_names=['maxEdges    ', 'nCells      '], data_type='integer')
    call io_add_var('mpas_mesh', 'cellsOnEdge',       long_name='Cell indices that saddle a given edge',                           units='',       dim_names=['TWO         ', 'nEdges      '], data_type='integer')
    call io_add_var('mpas_mesh', 'edgesOnCell',       long_name='Edge indices that surround a given cell',                         units='',       dim_names=['maxEdges    ', 'nCells      '], data_type='integer')
    call io_add_var('mpas_mesh', 'edgesOnEdge',       long_name='Edge indices that are used to reconstruct tangential velocities', units='',       dim_names=['maxEdges2   ', 'nEdges      '], data_type='integer')
    call io_add_var('mpas_mesh', 'verticesOnCell',    long_name='Vertex indices that surround a given cell',                       units='',       dim_names=['maxEdges    ', 'nCells      '], data_type='integer')
    call io_add_var('mpas_mesh', 'verticesOnEdge',    long_name='Vertex indices that saddle a given edge',                         units='',       dim_names=['TWO         ', 'nEdges      '], data_type='integer')
    call io_add_var('mpas_mesh', 'edgesOnVertex',     long_name='Edge indices that radiate from a given vertex',                   units='',       dim_names=['vertexDegree', 'nVertices   '], data_type='integer')
    call io_add_var('mpas_mesh', 'cellsOnVertex',     long_name='Cell indices that radiate from a given vertex',                   units='',       dim_names=['vertexDegree', 'nVertices   '], data_type='integer')
    call io_add_var('mpas_mesh', 'weightsOnEdge',     long_name='Weights used to reconstruct tangential velocities',               units='',       dim_names=['maxEdges2   ', 'nEdges      '], data_type='real(8)')
    call io_add_var('mpas_mesh', 'dvEdge',            long_name='Distance between the vertices that saddle a given edge',          units='m',      dim_names=['nEdges      '],                 data_type='real(8)')
    call io_add_var('mpas_mesh', 'dv1Edge',           long_name='Distance between the first vertex and edge location',             units='m',      dim_names=['nEdges      '],                 data_type='real(8)')
    call io_add_var('mpas_mesh', 'dv2Edge',           long_name='Distance between the second vertex and edge location',            units='m',      dim_names=['nEdges      '],                 data_type='real(8)')
    call io_add_var('mpas_mesh', 'dcEdge',            long_name='Distance between the cells that saddle a given edge',             units='m',      dim_names=['nEdges      '],                 data_type='real(8)')
    call io_add_var('mpas_mesh', 'angleEdge',         long_name='Angle between edge normal vector and local eastward direction',   units='radian', dim_names=['nEdges      '],                 data_type='real(8)')
    call io_add_var('mpas_mesh', 'areaCell',          long_name='Primary cell area',                                               units='m2',     dim_names=['nCells      '],                 data_type='real(8)')
    call io_add_var('mpas_mesh', 'areaTriangle',      long_name='Dual cell area',                                                  units='m2',     dim_names=['nVertices   '],                 data_type='real(8)')
    call io_add_var('mpas_mesh', 'kiteAreasOnVertex', long_name='Intersection area between areaCell and areaTriangle',             units='m2',     dim_names=['vertexDegree', 'nVertices   '], data_type='real(8)')
    call io_start_output('mpas_mesh')
    ...
    call io_output('mpas_mesh', 'indexToCellID',     mesh%indexToCellID)
    call io_output('mpas_mesh', 'lonEdge',           mesh%lonEdge)
    call io_output('mpas_mesh', 'latEdge',           mesh%latEdge)
    call io_output('mpas_mesh', 'xEdge',             mesh%xEdge)
    call io_output('mpas_mesh', 'yEdge',             mesh%yEdge)
    call io_output('mpas_mesh', 'zEdge',             mesh%zEdge)
    call io_output('mpas_mesh', 'indexToEdgeID',     mesh%indexToEdgeID)
    call io_output('mpas_mesh', 'lonVertex',         mesh%lonVertex)
    call io_output('mpas_mesh', 'latVertex',         mesh%latVertex)
    call io_output('mpas_mesh', 'xVertex',           mesh%xVertex)
    call io_output('mpas_mesh', 'yVertex',           mesh%yVertex)
    call io_output('mpas_mesh', 'zVertex',           mesh%zVertex)
    call io_output('mpas_mesh', 'indexToVertexID',   mesh%indexToVertexID)
    call io_output('mpas_mesh', 'nEdgesOnCell',      mesh%nEdgesOnCell)
    call io_output('mpas_mesh', 'nEdgesOnEdge',      mesh%nEdgesOnEdge)
    call io_output('mpas_mesh', 'cellsOnCell',       mesh%cellsOnCell)
    call io_output('mpas_mesh', 'cellsOnEdge',       mesh%cellsOnEdge)
    call io_output('mpas_mesh', 'edgesOnCell',       mesh%edgesOnCell)
    call io_output('mpas_mesh', 'edgesOnEdge',       mesh%edgesOnEdge)
    call io_output('mpas_mesh', 'verticesOnCell',    mesh%verticesOnCell)
    call io_output('mpas_mesh', 'verticesOnEdge',    mesh%verticesOnEdge)
    call io_output('mpas_mesh', 'edgesOnVertex',     mesh%edgesOnVertex)
    call io_output('mpas_mesh', 'cellsOnVertex',     mesh%cellsOnVertex)
    call io_output('mpas_mesh', 'weightsOnEdge',     mesh%weightsOnEdge)
    call io_output('mpas_mesh', 'dvEdge',            mesh%dvEdge)
    call io_output('mpas_mesh', 'dv1Edge',           mesh%dv1Edge)
    call io_output('mpas_mesh', 'dv2Edge',           mesh%dv2Edge)
    call io_output('mpas_mesh', 'dcEdge',            mesh%dcEdge)
    call io_output('mpas_mesh', 'angleEdge',         mesh%angleEdge)
    call io_output('mpas_mesh', 'areaCell',          mesh%areaCell)
    call io_output('mpas_mesh', 'areaTriangle',      mesh%areaTriangle)
    call io_output('mpas_mesh', 'kiteAreasOnVertex', mesh%kiteAreasOnVertex)
